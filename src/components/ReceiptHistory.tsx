...
export default function ReceiptHistory({ onBack }: ReceiptHistoryProps) {
  ...
  useEffect(() => {
    if (!userEmail) {
      setReceipts([]);
      setLoadError('Please sign in to view your receipts.');
      return;
    }
    if (!supabase) {
      setReceipts([]);
      setLoadError('Supabase is not configured. Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.');
      return;
    }
  ...
  }, [supabase, userEmail, receiptsVersion]);
